        ...
        {isOpen && (
          <div className="mt-2 bg-[#1a1a2e]/95 backdrop-blur-xl border border-white/[0.08] p-4 w-[300px] shadow-2xl shadow-black/50 rounded-2xl absolute right-4">
            ...
            <div className="mt-3 pt-3 border-t border-white/[0.06] space-y-2">
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Overlays</p>
              ...
              <button
                ...
                aria-label="Toggle crowd heatmap overlay"
              >
                <span className="flex items-center gap-2">🔥 Crowd Heatmap</span>
                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${heatmapEnabled ? "bg-orange-500/30 text-orange-200" : "bg-white/[0.06] text-gray-500"}`}>
                  {heatmapEnabled ? "ON" : "OFF"}
                </span>
              </button>

              <button
                onClick={() => onEventsToggle?.(!eventsEnabled)}
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl border transition-all text-xs font-semibold ${
                  eventsEnabled
                ...
                aria-label="Toggle campus events overlay"
              >
                <span className="flex items-center gap-2">📅 Campus Events</span>
                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${eventsEnabled ? "bg-blue-500/30 text-blue-200" : "bg-white/[0.06] text-gray-500"}`}>
                  {eventsEnabled ? "ON" : "OFF"}
                </span>
              </button>

              <button
                onClick={() => onSafetyToggle?.(!safetyEnabled)}
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl border transition-all text-xs font-semibold ${
                  safetyEnabled
                    ? "bg-amber-500/15 border-amber-500/30 text-amber-300"
                    : "bg-white/[0.04] border-white/[0.06] text-gray-400 hover:text-gray-200 hover:bg-white/[0.07]"
                }`}
                aria-pressed={safetyEnabled}
                aria-label="Toggle safety reports overlay"
              >
